...
namespace WindowsFormsApplication1
{
    ...
    class clDao:Provider
    {
        ...
        public DataTable getDoanhThukh(clDTO dto)
        {
           ...
           // string SQL = "SELECT TENKH,SUM(NHAP) AS TIENBANHANG, SUM(TRANHAP) AS TIENTRANHAP FROM (select  MAMH, (CASE WHEN T2.MANHAPXUAT<>'X' THEN 0 ELSE SUM(GIATIEN) END) AS XUAT,(CASE WHEN T2.MANHAPXUAT<>'TN' THEN 0 ELSE SUM(GIATIEN) END) AS TRANHAP FROM TONKHO as T2 WHERE NGAY BETWEEN '" + dto.NGAYBD + "' AND '" + dto.NGAYKT + "' AND T2.MAKHO='" + PublicVariable.MAKHO + "' group by MAMH,MAHD, MANHAPXUAT) AS T3, NHACUNGCAP,MATHANG WHERE MATHANG.MANCC=NHACUNGCAP.MANCC AND T3.MAMH=MATHANG.MAMH " + SQL1 + "  group by MATHANG.MANCC,TENNCC";
