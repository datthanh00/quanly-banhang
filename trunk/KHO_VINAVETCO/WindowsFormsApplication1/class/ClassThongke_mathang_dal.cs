...
//            String SQL = "select MATHANG.MAMH,KLDVT, TENMH, convert(varchar,NGAY,103)AS NGAY, MAHD,TENKHO,TENNCC,DONVITINH,TONDAU*KLDVT AS KLTONDAU,NHAP*KLDVT AS KLNHAP,TRANHAP*KLDVT AS KLTRANHAP,XUAT*KLDVT AS KLXUAT,TRAXUAT*KLDVT AS KLTRAXUAT,TONCUOI*KLDVT AS KLTONCUOI,TONDAU, NHAP,TRANHAP, XUAT,TRAXUAT, TONCUOI FROM MATHANG,TONKHO, NHACUNGCAP,DONVITINH,KHO WHERE MATHANG.MAMH=TONKHO.MAMH AND MATHANG.MANCC=NHACUNGCAP.MANCC AND MATHANG.MADVT=DONVITINH.MADVT AND TONKHO.MAKHO=KHO.MAKHO AND TONKHO.NGAY BETWEEN '" + dto.NGAYBDKHO + "' AND '" + dto.NGAYKTKHO + "' AND TONKHO.MAKHO='" + PublicVariable.MAKHO + "' " + MAMH + "";
            string BEGINYEAR = dto.NGAYBDKHO.Substring(0, 4);

            String SQL = "SELECT IDNHAP,T1.MAMH,TONDAU,NHAP,XUAT,TRANHAP,TRAXUAT,TONCUOI,KLDVT, TENMH+' - ' + QUYCACH AS TENMH, convert(varchar,NGAY,103)AS NGAY, MAHD,TENNCC,DONVITINH,TONDAU*KLDVT AS KLTONDAU,NHAP*KLDVT AS KLNHAP,TRANHAP*KLDVT AS KLTRANHAP,XUAT*KLDVT AS KLXUAT,TRAXUAT*KLDVT AS KLTRAXUAT,TONCUOI*KLDVT AS KLTONCUOI,LOHANG FROM (SELECT T2.MAMH,MAHD,LOHANG,NGAY,IDNHAP,(SELECT TONDAU FROM TONDAUMATHANG WHERE TONDAUMATHANG.MAMH=T2.MAMH AND CHOTKHO='01/01/" + BEGINYEAR + "')+(SELECT CASE WHEN SUM(NHAPXUAT) IS NULL THEN 0 ELSE SUM(NHAPXUAT) END FROM TONKHO WHERE TONKHO.MAMH=T2.MAMH AND TONKHO.NGAY BETWEEN '01/01/" + BEGINYEAR + "' AND '" + dto.NGAYBDKHO + "' ) AS TONDAU,"
            ...
            + " (SELECT TONDAU FROM TONDAUMATHANG WHERE TONDAUMATHANG.MAMH=T2.MAMH  AND CHOTKHO='01/01/" + BEGINYEAR + "')+(SELECT CASE WHEN SUM(NHAPXUAT) IS NULL THEN 0 ELSE SUM(NHAPXUAT) END FROM TONKHO WHERE TONKHO.MAMH=T2.MAMH AND TONKHO.NGAY BETWEEN '01/01/" + BEGINYEAR + "' AND '" + dto.NGAYKTKHO + "' ) AS TONCUOI "
    ...
    //---------------------------------------------
        ...
        public DataTable load_ct_mathang2(Class_DTO_ThongKe dto)
        {
            string SQL = "";
                ...
                {//load tat ca
                    ...
                    + " FROM THUE,NHACUNGCAP,MATHANG,DONVITINH WHERE MATHANG.TINHTRANG='True' AND THUE.MATH = MATHANG.MATH  and NHACUNGCAP.MANCC=MATHANG.MANCC and DONVITINH.madvt=MATHANG.madvt and MATHANG.MAKHO='" + PublicVariable.MAKHO + "' ";
                }
                if (dto.Loai_HT == "1")
                {//con han
                    SQL = " SELECT MATHANG.MAMH,MATHANG.MANCC,TENNCC,KLDVT,THUE.MATH,SOTHUE,MATHANG.MANH,TENNCC,TENMH,DONVITINH.MADVT,DONVITINH,SOLUONGMH,SOLUONGMH*KLDVT AS KHOILUONG,HANSUDUNG,GIAMUA,CAST(((GIAMUA*SOLUONGMH) + (SOTHUE * SOLUONGMH * GIAMUA) / 100) as  NUMERIC(18,0))  as thanhtien,GIABAN,CAST((GIABAN*SOLUONGMH + (SOTHUE * SOLUONGMH * GIABAN) / 100) as  NUMERIC(18,0)) as thanhtienban,MOTA"
                   + " FROM THUE,NHACUNGCAP,MATHANG,DONVITINH WHERE MATHANG.TINHTRANG='True' AND THUE.MATH = MATHANG.MATH  and NHACUNGCAP.MANCC=MATHANG.MANCC and DONVITINH.madvt=MATHANG.madvt AND MATHANG.MAMH='" + dto.MAMH + "' and MATHANG.MAKHO='" + PublicVariable.MAKHO + "' ";
                }
                if (dto.Loai_HT == "2")
                {//het han
                    SQL = " SELECT MATHANG.MAMH,MATHANG.MANCC,TENNCC,KLDVT,THUE.MATH,SOTHUE,MATHANG.MANH,TENNCC,TENMH,DONVITINH.MADVT,DONVITINH,SOLUONGMH,SOLUONGMH*KLDVT AS KHOILUONG,HANSUDUNG,GIAMUA,CAST(((GIAMUA*SOLUONGMH) + (SOTHUE * SOLUONGMH * GIAMUA) / 100) as  NUMERIC(18,0))  as thanhtien,GIABAN,CAST((GIABAN*SOLUONGMH + (SOTHUE * SOLUONGMH * GIABAN) / 100) as  NUMERIC(18,0)) as thanhtienban,MOTA"
                    + " FROM THUE,NHACUNGCAP,MATHANG,DONVITINH WHERE MATHANG.TINHTRANG='True' AND THUE.MATH = MATHANG.MATH  and NHACUNGCAP.MANCC=MATHANG.MANCC and DONVITINH.madvt=MATHANG.madvt AND MATHANG.MANCC='" + dto.MANCC + "' and MATHANG.MAKHO='" + PublicVariable.MAKHO + "' ";
                }
        ...
        {
            string SQL = " SELECT KHOASO.ID,NGHIEPVU,TINHTRANG,GHICHU,NGAY FROM KHOASO,KHOASOTHEOKHO WHERE KHOASO.ID=KHOASOTHEOKHO.ID AND MAKHO='"+PublicVariable.MAKHO+"'";
            return getdata(SQL);
        }
        ...
        public DataTable load_ct_mathang_lo(Class_DTO_ThongKe dto)
        {
            ...
            SQL1 = SQL1 + " AND MATHANG.TINHTRANG=1 AND (TONKHO >0 OR (SOLUONGMH=0 AND LOHANG='" + PublicVariable.LOHANG + "'))";

            SQL = "SELECT MATHANG.MAMH,MATHANG.MANCC,TENNCC,KLDVT,THUE.MATH,SOTHUE,MATHANG.MANH,TENNCC,TENMH+' - ' + QUYCACH AS TENMH,DONVITINH.MADVT,DONVITINH,TONKHO AS SOLUONGMH,TONKHO*KLDVT as KHOILUONG,HSD AS HANSUDUNG, cast(HSD-GETDATE() as int) as NGAYSUDUNG,KHOHANG.GIAMUA,LOHANG, (khohang.GIAMUA*TONKHO) as thanhtien, MOTA "
        ...
        {
            String SQL = "SELECT convert(varchar,CHOTKHO,103)AS CHOTKHO FROM TONDAUMATHANG,MATHANG WHERE MATHANG.MAMH=TONDAUMATHANG.MAMH AND MATHANG.MAKHO='" + PublicVariable.MAKHO + "' GROUP BY CHOTKHO ORDER BY CHOTKHO DESC";
            return getdata(SQL);
        }
        ...
        {
            String SQL = "select 	MAMH AS MASANPHAM,TENMH AS TENSANPHAM,TENNCC from  MATHANG,NHACUNGCAP where MATHANG.MANCC=NHACUNGCAP.MANCC and MATHANG.TINHTRANG=1 AND mathang.makho='" + PublicVariable.MAKHO + "'";
            return getdata(SQL);
        }
        public DataTable dtGetsanpham2()
        {
            String SQL = "select 	MAMH ,TENMH,TENNCC from  MATHANG,NHACUNGCAP where MATHANG.MANCC=NHACUNGCAP.MANCC and MATHANG.TINHTRANG=1 AND mathang.makho='" + PublicVariable.MAKHO + "'";
            return getdata(SQL);
        }
    }
}
