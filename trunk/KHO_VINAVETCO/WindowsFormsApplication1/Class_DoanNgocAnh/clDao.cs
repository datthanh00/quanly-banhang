    ...
    class clDao:Provider
    {
        ...
        public DataTable getTonKhoTTngay(clDTO dto)
        {
          ...
          /*  String SQL = " SELECT  convert(varchar,'" + dto.NGAYBD + "',103) AS NGAY, SOLUONGMH-SOLUONGMH AS CHENHLECH,T2.MAMH,TENMH, TENNCC, DONVITINH,KLDVT,SOLUONGMH*kldvt as KLTONTT,(SOLUONGMH-(NHAP+TRAXUAT-XUAT-TRANHAP))*KLDVT AS KLTONDAU,NHAP*KLDVT AS KLNHAP,TRANHAP*KLDVT AS KLTRANHAP,XUAT*KLDVT AS KLXUAT,TRAXUAT*KLDVT AS KLTRAXUAT,SOLUONGMH*KLDVT AS KLTONCUOI,SOLUONGMH AS TONCUOI,SOLUONGMH AS TONTT,SOLUONGMH*giamua AS THANHTIENTONCUOI, NHAP,NHAP*GIAMUA AS THANHTIENNHAP, XUAT,XUAT*GIABAN AS THANHTIENXUAT,TRANHAP,TRANHAP*GIAMUA AS THANHTIENTRANHAP ,TRAXUAT,TRAXUAT*GIABAN AS THANHTIENTRAXUAT,SOLUONGMH-(NHAP+TRAXUAT-XUAT-TRANHAP) AS TONDAU,(SOLUONGMH-(NHAP+TRAXUAT-XUAT-TRANHAP))* GIAMUA AS THANHTIENTONDAU,GIAMUA,GIABAN FROM "
           ...
           /* String SQL = "SELECT T2.*,TONKHONGAY AS TONCUOI FROM (select MATHANG.MAMH,TENMH,TENNCC,KLDVT,DONVITINH,GIAMUA,SUM(TRAXUAT)AS TRAXUAT,SUM(NHAP) AS NHAP,SUM(TRANHAP) AS TRANHAP,SUM(XUAT) AS XUAT,SUM(NHAP*GIANHAP) AS THANHTIENNHAP,SUM(TRANHAP*GIANHAP) AS THANHTIENTRANHAP,SUM(XUAT*GIAXUAT) AS THANHTIENXUAT, SUM(TRAXUAT*GIAXUAT) AS THANHTIENTRAXUAT"
            + " FROM TONKHO, NHACUNGCAP,MATHANG,DONVITINH WHERE TONKHO.MAMH=MATHANG.MAMH AND NHACUNGCAP.MANCC=MATHANG.MANCC AND MATHANG.MADVT=DONVITINH.MADVT AND MATHANG.TINHTRANG='TRUE' AND NGAY='" + dto.NGAYBDKHO + "' AND MATHANG.MAKHO='" + PublicVariable.MAKHO + "' " + SQL1 + " GROUP BY  MATHANG.MAMH,TENMH,TENNCC,KLDVT,DONVITINH,SOLUONGMH,TONCUOINGAY,GIABAN,GIAMUA) AS T2, (SELECT * FROM TONKHOTT WHERE  NGAY='" + dto.NGAYBDKHO + "' AND MAKHO='" + PublicVariable.MAKHO + "') AS TONKHOTT WHERE TONKHOTT.MAMH=T2.MAMH";
            */

            String SQL = "SELECT *, TONDAU*KLDVT AS KLTONDAU,SOLUONGMH-SOLUONGMH AS CHENHLECH,SOLUONGMH AS TONTT, NHAP*KLDVT AS KLNHAP,TRANHAP*KLDVT AS KLTRANHAP, XUAT*KLDVT AS KLXUAT, TRAXUAT*KLDVT AS KLTRAXUAT,TONCUOI*KLDVT AS KLTONCUOI ,TONDAU*GIABAN AS THANHTIENTONDAU, NHAP*GIABAN AS THANHTIENNHAP,TRANHAP*GIABAN AS THANHTIENTRANHAP, XUAT*GIABAN AS THANHTIENXUAT, TRAXUAT*GIABAN AS THANHTIENTRAXUAT, TONCUOI*GIABAN AS THANHTIENTONCUOI"
        ...
        public DataTable getcpmuahangngay(clDTO dto)
        {
            ...
            string SQL = "SELECT NGAYNHAP, SUM(NHAP) AS NHAP, SUM(TRAXUAT) AS TRAXUAT FROM(select NGAY AS NGAYNHAP, (CASE WHEN T2.MANHAPXUAT<>'N' THEN 0 ELSE SUM(GIATIEN) END) AS NHAP,(CASE WHEN T2.MANHAPXUAT<>'TX' THEN 0 ELSE SUM(GIATIEN) END) AS TRAXUAT FROM TONKHO as T2 WHERE NGAY BETWEEN '" + dto.NGAYBD + "' AND '" + dto.NGAYKT + "' AND T2.MAKHO='" + PublicVariable.MAKHO + "' group by NGAY, MANHAPXUAT) AS T3 group by NGAYNHAP";
            return getdata(SQL);
        }
        ...
        public DataTable getcpmuahangncc(clDTO dto)
        {
            ...
            string SQL = "SELECT MATHANG.MANCC,TENNCC,SUM(NHAP) AS TIENMUAHANG, SUM(TRAXUAT) AS TIENTRAXUAT FROM (select  MAMH, (CASE WHEN T2.MANHAPXUAT<>'N' THEN 0 ELSE SUM(GIATIEN) END) AS NHAP,(CASE WHEN T2.MANHAPXUAT<>'TX' THEN 0 ELSE SUM(GIATIEN) END) AS TRAXUAT FROM TONKHO as T2 WHERE NGAY BETWEEN '" + dto.NGAYBD + "' AND '" + dto.NGAYKT + "' AND T2.MAKHO='" + PublicVariable.MAKHO + "' group by MAMH, MANHAPXUAT) AS T3, NHACUNGCAP,MATHANG WHERE MATHANG.MANCC=NHACUNGCAP.MANCC AND T3.MAMH=MATHANG.MAMH " + SQL1 + "  group by MATHANG.MANCC,TENNCC";
            return getdata(SQL);
        }
        public DataTable getDoanhThungay(clDTO dto)
        {
            ...
            string SQL = "SELECT NGAYXUAT, SUM(XUAT) AS XUAT, SUM(TRANHAP) AS TRANHAP FROM(select NGAY AS NGAYXUAT, (CASE WHEN T2.MANHAPXUAT<>'X' THEN 0 ELSE SUM(GIATIEN) END) AS XUAT,(CASE WHEN T2.MANHAPXUAT<>'TN' THEN 0 ELSE SUM(GIATIEN) END) AS TRANHAP FROM TONKHO as T2 WHERE NGAY BETWEEN '" + dto.NGAYBD + "' AND '" + dto.NGAYKT + "' AND T2.MAKHO='" + PublicVariable.MAKHO + "' group by NGAY, MANHAPXUAT) AS T3 group by NGAYXUAT";
